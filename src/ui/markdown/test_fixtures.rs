...
mod tests {
    ...
    #[test]
    fn fixtures_are_valid_messages() {
        // Smoke test: ensure all fixtures create valid messages
        let _ = single_block();
        let _ = multiple_blocks();
        let _ = blocks_across_messages();
        let _ = nested_in_list();
        let _ = wrapped_code();
        let _ = empty_block();
        let _ = no_language_tag();
        let _ = adjacent_to_text();
        let _ = user_message_with_code();
        let _ = code_and_links();
        let _ = various_languages();
    }

    #[test]
    fn fixtures_have_expected_roles() {
        assert_eq!(single_block().role, ROLE_ASSISTANT);
        assert_eq!(user_message_with_code().role, ROLE_USER);

        let messages = blocks_across_messages();
        assert_eq!(messages[0].role, ROLE_USER);
        assert_eq!(messages[1].role, ROLE_ASSISTANT);
    }

    #[test]
    fn fixtures_contain_code_fences() {
        assert!(single_block().content.contains("```"));
        assert!(multiple_blocks().content.contains("```rust"));
        assert!(no_language_tag().content.starts_with("```\n"));
    }
}
